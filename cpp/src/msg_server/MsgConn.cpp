...
#ifdef NO_DB
    char user_name[32] = "dongxie";
    char nick_name[32] = "spider man";
    char avatar_url[32] = "www.baidu.com";
    char title[32] = "super man";
    char postion[32] = "china";
    char telphone[32] = "110";
    char email[32] = "dongxie.mogujie.com";

    user_info_t user;
    user.user_id = 10000;
    user.nick_name_len = strlen(nick_name);
    user.nick_name = nick_name;
    user.avatar_len = strlen(avatar_url);
    user.avatar_url = avatar_url;
    user.title_len = strlen(title);
    user.title = title;
    user.position_len = strlen(postion);
    user.position = postion;
    user.role_status = 0;
    user.sex = 0;
    user.depart_id = 1;
    user.job_num = 1000;
    user.telphone_len = strlen(telphone);
    user.telphone = telphone;
    user.email_len = strlen(email);
    user.email = email;
    user.create_time = 123456789;
    CImPduValidateResponse pduRsp(user_name, 0, &user, attach_data.GetLength(), attach_data.GetBuffer(),
                                  pPdu->GetReserved());
    
	pDbConn->HandlePdu((CImPdu*)&pduRsp);
#else
...
#ifdef NO_DB
        department_info_t depart;
        char szTitle[32] = "swat";
        char szDescription[32] = "super man";
        depart.depart_id = 1;
        depart.title_len = strlen(szTitle);
        depart.title = szTitle;
        depart.description_len = strlen(szDescription);
        depart.description = szDescription;
        depart.parent_depart_id = 0;
        depart.leader = 11111;
        depart.status = 0;
        
        CImPduClientDepartmentResponse pdu2(1, &depart);
...
void CMsgConn::SendUserActionLog(uint32_t action_type)
{
	...
	//log("SendUserActionLog, user_id=%u, type=%u\n", GetUserId(), action_type);
	//CDBServConn* pDBConn = get_db_serv_conn_for_login();
	//if (pDBConn) {
	//	CImPduUserActionLog pdu(0, 0, m_peer_ip.c_str(), GetUserId(), action_type,
	//			NULL, NULL, NULL, m_client_version.c_str());
	//	pDBConn->SendPdu(&pdu);
	//}
}
