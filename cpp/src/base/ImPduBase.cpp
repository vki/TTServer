...
void CImPdu::WriteHeader()
{
	...
	CByteStream::WriteUint16(buf + 8, m_pdu_header.module_id);
	CByteStream::WriteUint16(buf + 10, m_pdu_header.command_id);
	CByteStream::WriteUint16(buf + 12, m_pdu_header.error);
	CByteStream::WriteUint16(buf + 14, m_pdu_header.reserved);
}
...
{
	uchar_t* buf = GetBuffer();
	CByteStream::WriteUint16(buf + 14, reserved);
}
