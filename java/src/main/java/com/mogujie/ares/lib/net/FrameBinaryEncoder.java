...
public class FrameBinaryEncoder extends SimpleChannelDownstreamHandler {
    ...
    public void writeRequested(ChannelHandlerContext ctx, MessageEvent e)
        ...
        if (dataBuffer != null) {
            ...
            char[] tc = Character.toChars(request.getVersion());
            headBuffer.putChar(tc[0]);// 消息version, 1个字节
            tc = Character.toChars(request.getFlag());
            headBuffer.putChar(tc[0]);// 标记: 分拆数据包，压缩
            tc = Character.toChars(request.getServiceId());
            headBuffer.putChar(tc[0]); // 服务号, 后端这边固定1000
            tc = Character.toChars(request.getCommandId());
            headBuffer.putChar(tc[0]); // 命令号
            tc = Character.toChars(request.getError());
            headBuffer.putChar(tc[0]); // 服务器错误
            ...
            headBuffer.put((byte) request.getFlag());// 标记: 分拆数据包，压缩
